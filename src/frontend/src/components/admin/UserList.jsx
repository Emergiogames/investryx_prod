...
import { adminUserList, adminUserBlock } from "../../services/admin/apiMethods";
import { toast } from "react-toastify";
import AdminViewUser from "./ViewUser";
...
const UserList = () => {
    ...
    const handleUserBlock = () => {
        adminUserBlock({ id: userIdToBlock, type: "profile" })
            .then((response) => {
                const data = response.data;
                toast[blockStatus === "block" ? "error" : "info"](data.message);

                // Update userStates locally
                setUserStates((prevUsers) =>
                    prevUsers.map((user) =>
                        user.id === userIdToBlock ? { ...user, block: blockStatus === "block" } : user
                    )
                );
            })
            .catch((error) => {
    ...
    return (
        <>
            <div className="">
                <div className=" flex justify-center font-serif text-xl mt-6 ">Users List</div>
                                            ...
                                            >
                                                ...
                                                <td className="px-6 py-4 text-center">
                                                    {user.block ? (
                                                        <button
                                                            type="button"
                                                            onClick={() => confirmBlockUser(user.id, "unblock")}
                                                        ...
                                                        <button
                                                            type="button"
                                                            onClick={() => confirmBlockUser(user.id, "block")}
