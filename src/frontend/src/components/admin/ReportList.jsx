...
const ReportList = () => {
    ...
    const filteredReports = reportStates.filter((report) =>
        report?.reason_type.toLowerCase().includes(searchTerm.toLowerCase())
    );
    ...
    const handleReportAction = () => {
        adminPostBlock({ id: reportIdToAction , type: "post" })
            .then((response) => {
                const data = response.data;
                toast[actionStatus === true ? "info" : "error"](data.message);
                fetchReports();
            })
            .catch((error) => {
                console.log(error);
    ...
    return (
        <>
            <div className="w-9/12">
                <div className=" flex justify-center items-center font-serif text-xl mt-6">Reports List</div>
                                        ...
                                        {currentReports.map((report, index) => (
                                            ...
                                            >
                                                ...
                                                <td className="px-6 py-4 text-center">
                                                    {report.report_post?.image1 && (
                                                        <img
                                                            src={report.report_post.image1}
                                                            alt={report.report_post.image1}
                                                            className="w-14 h-14 object-cover rounded-lg"
                                                        />
                                                    )}
                                                </td>
                                                <td className="px-6 py-4">{report.report_post?.title || "N/A"}</td>
                                                <td className="px-6 py-4 text-center">
                                                    {report?.report_post?.name || "N/A"}
                                                </td>

                                                <td className="px-6 py-4 text-center">{report?.reason}</td>
                                                <td className="px-6 py-4 text-center">{report?.reason_type || "Nil"}</td>
                                                <td className="px-6 py-4 text-center">
                                                    {report.report_post?.block ? "Blocked" : "Unblocked"}
                                                </td>
                                                <td className="px-6 py-4 text-center">
                                                    {report.report_post?.block ? (
                                                        <button
                                                            type="button"
                                                            onClick={() =>
                                                                confirmActionReport(report?.report_post?.id, "unblock")
                                                            }
                                                        ...
                                                        <button
                                                            type="button"
                                                            onClick={() =>
                                                                confirmActionReport(report?.report_post?.id, "block")
                                                            }
