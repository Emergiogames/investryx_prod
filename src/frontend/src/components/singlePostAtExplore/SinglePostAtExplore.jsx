...
const SinglePostAtExplore = ({ props, loading }) => {
    ...
    useEffect(() => {
        const isSaved = wishPosts.some((wishPost) => wishPost.id === posts.id);
        setIsSavedByUser(isSaved);
    }, [wishPosts, posts.id]);

    const navigate = useNavigate();
    const profileType = useSelector((state) => state.auth.profile);
    ...
    const handleViewPost = (post) => {
        switch (profileType) {
            ...
            case "investor":
                ...
                break;
            case "franchise":
                navigate(`/view-post-fra/${post.id}`, { state: { post } });
            case "advisor":
    ...
    return !loading && posts?.length > 0 ? (
        <div className=" relative px-4 md:px-8 lg:px-16">
            ...
            <Swiper
                ...
                className="swiper-wrapper  mb-8 max-w-[70rem] mx-auto"
            >
                {posts.map((post) => (
                            ...
                            {post.entity_type === "advisor" ? (
                                <div className="relative mb-4">
                                    {post.logo ? (
                                        ...
                                        <div
                                            onClick={() => handleViewPost(post)}
                                            className="h-60 lg:h-80 w-full object-cover flex justify-center items-center rounded-2xl transition-transform duration-300 transform group-hover:scale-105"
                                        >
                                ...
                                <div onClick={() => handleViewPost(post)} className="relative  mb-4">
                                    {post.logo ? (
                                        <img
                                            className="w-full h-60 lg:h-80 object-cover rounded-2xl transition-transform duration-300 transform group-hover:scale-105"
                                            src={`${BASE_URL}${post.logo}`}
                                            alt={post.title || "Post Image"}
                                        />
                                    ) : post.image1 ? (
                                    ...
                                    {/* <div className="absolute bottom-3 left-3 inline-flex items-center rounded-lg bg-white p-2 shadow-md">
                                        <IoBookmarks
                                            ...
                                            className={isSavedByUser ? "text-amber-300" : ""}
                                        />
                                    </div> */}
                                </div>
                            )}
        ...
        <div className="flex justify-center">No posts to show</div>
    );
};

export default SinglePostAtExplore;
