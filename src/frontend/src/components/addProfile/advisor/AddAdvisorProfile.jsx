...
const AdvisorProfileSchema = Yup.object().shape({
  name: Yup.string()
    .matches(
      /^[A-Za-z\s]+$/,
  ...
  .matches(/^\d+$/, "Experience must contain only numbers without spaces")
...
function AddAdvisorProfile() {
  ...
  const handleSubmit = (values, { setSubmitting, resetForm }) => {
    const updatedValues = { ...values, type: "advisor" ,interest: selectedOptions  };
    console.log("updataed form submission :;", updatedValues);

    addPost(updatedValues)
      .then((response) => {
        if (response.data.status === true) {
          toast.success("Added Advisor Profile Successfully!");
        } else {
  ...
  //only to handle multi options, in one field of formik
  ...
  return (
    <Formik
      ...
      validationSchema={AdvisorProfileSchema}
                ...
                <div className="flex items-center justify-center p-12">
                  <div className="mx-auto w-full max-w-[550px] bg-amber-100">
                    ...
                    <div className="mb-5">
                      ...
                      {/* Advisor Area of interest (multiselect) */}
                      <div className="mb-5">
      <label
        htmlFor="interest"
        className="mb-3 block text-base font-medium text-[#07074D]"
        ...
        {selectedOptions.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {selectedOptions.map((option, index) => (
              <div
                key={index}
                className="flex items-center bg-gray-200 text-gray-700 py-1 px-3 rounded-lg"
              >
                <span>{option}</span>
                <button
                  type="button"
...
export default AddAdvisorProfile;
