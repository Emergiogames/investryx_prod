...
function AddInvestorProfile() {
  ...
  const handleSubmit = (values, { setSubmitting, resetForm }) => {
    ...
    // Append the additional "type" field
    formData.append("type", "investor");
  
    console.log("FormData submission:", values);
  
    addPost(formData)
      .then((response) => {
        if (response.data.status === true) {
          toast.success("Added Investor Profile Successfully!");
  ...
  const handleFileChange = (event, setFieldValue) => {
    ...
    if (file) {
      setFieldValue("image", file); // Correctly updating Formik's state
      setImagePreview(URL.createObjectURL(file)); // Preview logic remains
    }
  };
