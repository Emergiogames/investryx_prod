...
function ChatList() {
    ...
    console.log("audio bloobbbeeee", recordedAudioBlob);
    ...
    const handleSendAudioMessage = (recordedAudioBlob) => {
        if (recordedAudioBlob && websocket && websocket.readyState === WebSocket.OPEN) {
            const reader = new FileReader();

            reader.readAsDataURL(recordedAudioBlob); // Convert Blob to Base64
            reader.onloadend = () => {
                ...
                const audioMessage = {
                    roomId,
                    token,
                    audio: base64Audio, // Sending as Base64 string
                };
    ...
    const handleSendAttachment = (file) => {
        if (file && websocket && websocket.readyState === WebSocket.OPEN) {
            const reader = new FileReader();
            ...
            reader.onloadend = () => {
                ...
                // Match Flutter's attachment format
                const attachment = {
                    fileName: file.name,
                    data: base64Data, // Use "data" instead of "base64Data"
                    type: file.type.startsWith("image/") ? "image" : "file",
                    fileExtension: "." + file.name.split(".").pop().toLowerCase(), // Include leading dot
                    size: file.size,
                };
                ...
                const attachmentMessage = {
                    ...
                    attachment: JSON.stringify(attachment), // Encode attachment as a JSON string
                    roomId,
                    time: new Date().toISOString(), // Use "time" instead of "date"
                };

                console.log("Sending attachment:", attachmentMessage);
                websocket.send(JSON.stringify(attachmentMessage)); // Convert to string before sending
            };
        }
    };
    ...
    //           roomId,
    //           token,
    //           attachment: base64File, // Sending as Base64 string
    //           attachmentType: file.type, // Include file type (e.g., image/png, application/pdf)
    //           fileName: file.name, // Include file name
    //         };

    //         console.log("Sending attachment:", attachmentMessage);
    //         websocket.send(JSON.stringify(attachmentMessage));
    //       };
    //     }
    //   };

    // Handle file selection and sending attachments
    const handleFileChange = (event) => {
        const files = event.target.files;
        if (files.length > 0) {
            for (let file of files) {
                handleSendAttachment(file); // Send each selected file
            }
        }
    };
                    ...
                    <div class="flex flex-col flex-auto h-full p-6 ">
                        ...
                        <div class="flex flex-col flex-auto flex-shrink-0 rounded-2xl bg-gray-100 h-full p-4">
                            {loading ? (
                                ...
                                <Messages messages={messages} user={user} sendedByImg={sendedByImg} />
                            )}
