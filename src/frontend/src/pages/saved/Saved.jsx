    ...
    getWishList()
      .then((response) => {
        ...
        const categorizedPosts = {
          business: wishlistData.filter(
            (post) => post.entity_type === "business"
          ),
          investor: wishlistData.filter(
            (post) => post.entity_type === "investor"
          ),
          franchise: wishlistData.filter(
            (post) => post.entity_type === "franchise"
          ),
          advisor: wishlistData.filter(
            (post) => post.entity_type === "advisor"
          ),
        };
  ...
  const handleDelete = (postId, entityType) => {
    ...
    deleteWishList(postId)
      .then((response) => {
        ...
        setPosts((prevPosts) => ({
          ...prevPosts,
          [entityType]: prevPosts[entityType].filter(
            (post) => post.id !== postId
          ),
        }));
      })
